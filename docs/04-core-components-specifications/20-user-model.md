...
2. **Route planning**: The manager calls an external route planner (`Planner`) with `plan(org, dst, dept)`.
...
- **Reservation vs Direct Depart**:
  ...
  - Otherwise, legs are built as direct `Trip` tasks (e.g., walking or services without reservations).
- **Waiting**:
